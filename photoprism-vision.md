...
**vision.yml** (`../photoprism/storage/config/vision.yml`) — PhotoPrism's AI model config. All three models use `Engine: vision` pointing at `http://172.17.0.1:5050` (docker0 bridge IP). Caption uses kosmos-2 (local PyTorch), labels use llama3.2-vision (Ollama on GPU), NSFW uses nsfw_image_detector (local PyTorch). Note: `host.docker.internal` resolves correctly but times out on this host — use `172.17.0.1` instead.
